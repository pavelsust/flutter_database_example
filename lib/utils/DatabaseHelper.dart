...
class DatabaseHelper {
  ...
  Future<int> updateNote(Note note) async{
    var database = await this.database;
    var result = database.update(TABLE_NAME, note.toMap() , where: '$ID = ?' , whereArgs: [note.id]);
    return result;
  }
  ...
  Future<int> deleteNote(Note note) async{
    var database = await this.database;
    var result =  await database.delete(TABLE_NAME , where: '$ID = ?' , whereArgs: [note.id]);
    return result;
  }
  ...
  Future<int> getCount() async{
    var database = await this.database;
    List<Map<String , dynamic>> x = database.rawQuery('SELECT COUNT * from $TABLE_NAME') as List<Map<String, dynamic>>;
